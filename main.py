...
# ==============================================================================
# --- گرفتن محصولات هر دسته فقط محصولات موجود ---
# ==============================================================================
...
def get_products_from_category_page(session, category_id, max_pages=30, delay=0.5):
    ...
    while True:
        if page_num == 1:
            ...
            logger.info(f"  - دریافت صفحه اول: {url}")
            response = session.get(url, timeout=30)
            if response.status_code != 200:
                logger.error(f"    - خطا در دریافت صفحه اول: {response.status_code}")
                break
            soup = BeautifulSoup(response.text, 'lxml')
            product_blocks = soup.select("div.col-lg-3.col-md-4.col-sm-6.goods-p")
            logger.info(f"    - تعداد محصولات صفحه اول: {len(product_blocks)}")
            if not product_blocks:
                break
            for block in product_blocks:
                try:
                    # منطق جدید شناسایی موجودی
                    bell = block.select_one("i.far.fa-bell")
                    is_out_of_stock = False
                    if bell:
                        bell_parent = bell.parent
                        bell_text = bell_parent.get_text(strip=True) if bell_parent else ""
                        if "0 عدد در انبار باقیست" in bell_text:
                    ...
                    logger.warning(f"      - خطا در پردازش محصول صفحه اول: {e}")
        else:
        ...
        # اگر تعداد محصول کمتر از page_size بود یعنی آخرین صفحه است
        if (page_num == 1 and len(product_blocks) < page_size) or (page_num > 1 and len(goods) < page_size):
            break
        page_num += 1
        time.sleep(random.uniform(delay, delay + 0.2))
