    ...
    while page_num <= max_pages:
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                break
            soup = BeautifulSoup(response.text, 'lxml')
            # انتخاب‌گر ترکیبی: اول goods-body، fallback به goods_item
            product_blocks = soup.select('div.row.goods-body')
            ...
            for block in product_blocks:
                try:
                    ...
                    if product_id in seen_product_ids:
                        ...
                        continue
                    name_tag = block.select_one("h1.name-heading") or block.select_one("span.goods-record-title")
                    name = name_tag.text.strip() if name_tag else "نامشخص"
                    price_tag = block.select_one("span.price") or block.select_one("span.goods-record-price")
                    price_raw = price_tag.text.strip() if price_tag else "0"
                    price = re.sub(r'[^\d]', '', price_raw) or "0"
                    image_tag = block.select_one("img.center-block") or block.select_one("img.goods-record-image")
                    image_url = image_tag.get('src', '') or image_tag.get('data-src', '') if image_tag else ''
    ...
    for block in product_blocks:
        try:
            ...
            if product_id and product_id not in seen:
                seen.add(product_id)
                name = block.select_one("h1.name-heading") or block.select_one("span.goods-record-title")
                name = name.text.strip() if name else "نامشخص"
