...
def find_category_with_available_products(session, all_categories):
    """به صورت خودکار یک دسته‌بندی با محصولات موجود پیدا می‌کند."""
        ...
        try:
            ...
            for block in product_blocks:
                if 'noCount' not in block.get('class', []):
                    print(f"\n✅ دسته‌بندی '{category['name']}' (ID: {category['id']}) با محصول موجود پیدا شد!")
                    return category
        except Exception:
            continue
    print("❌ متاسفانه هیچ دسته‌بندی با محصول موجود در نمونه بررسی شده پیدا نشد.")
    return None
...
def main():
    ...
    # به صورت خودکار یک دسته‌بندی مناسب پیدا می‌کند
