...
CATEGORIES_API_URL = f"{API_BASE_URL}/categories/megamenu"
...
def sync_categories():
    print("="*20 + " فاز ۰: همگام‌سازی دسته‌بندی‌ها " + "="*20)
...
def get_all_products_from_category(category_id):
    ...
    while True:
        ...
        data = make_api_request(PRODUCTS_LIST_URL_TEMPLATE.format(category_id=category_id), params=params)
...
def process_and_send_variations(wc_product_map, variations_map):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for task in tasks:
            ...
            delete_ids = [v['id'] for v in requests.get(f"{WC_PRODUCTS_API_URL}/{task['id']}/variations?per_page=100", auth=(WC_CONSUMER_KEY, WC_CONSUMER_SECRET), verify=False).json()]
            if delete_ids:
...
def main():
    ...
    for product in tqdm(unique_products, desc="Preparing Products"):
